...
from utils.sampling import sample_dirichlet
...
from utils.tsne_eval import tsne_evaluation
    ...
    for epoch in range(args.epochs):
        ...
        for client_num, idx in enumerate(idxs_users):
            ...
            if args.method == 'dualoptim':
                ...
                if epoch < epoch_of_stage1:
                    ...
                    # print(metrics)
                    metrics = metrics / num
                    metrics = np.nan_to_num(metrics, nan=0.1)
                    # print("^^^^^^")
                    # print(metrics)
                ...
                if epoch < epoch_of_stage1:  # stage 1
                    ...
                    if idx in local_weights.clean_clients:
                        ...
                        w, loss, percentage_correct, before_percentage_correct, after_percentage_correct, merged_percentage_correct, f_k = local.train_stage2(
                            net=copy.deepcopy(net_glob).to(args.device), global_net=copy.deepcopy(net_glob).to(args.device), weight_kd=weight_kd, current_round=epoch, f_G=copy.deepcopy(f_G).to(args.device), min_gapp=min_gapp, max_gapp=max_gapp)
        ...
        if feature_locals:
            ...
            for i in feature_locals:
                f_G = f_G.to(args.device)
                i = i.to(args.device)
                f_k = f_k.to(args.device)
        ...
        gap = torch.ones((args.num_classes, args.num_classes), device=args.device) * 1e9
        for i in range(args.num_classes):
            for j in range(i):
                dis = torch.norm(f_G[i] - f_G[j], p=2)
                gap[i, j] = dis
                gap[j, i] = dis
        min_gapp = torch.min(gap[torch.triu(torch.ones_like(gap), diagonal=1) > 0])
        max_gapp = torch.max(gap[torch.triu(torch.ones_like(gap), diagonal=1) > 0])
        ...
        if epoch % 10 == 0 or epoch == (args.epochs-1):
            test_loader = DataLoader(dataset_test, batch_size=args.test_bs, shuffle=False)
            save_dir = f"tsne/{args.method}"
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"tsne_epoch_{epoch}.png")

            tsne_evaluation(net_glob, test_loader, save_path=save_path)

            # results2 = dict(train_acc2=train_acc2, train_loss2=train_loss2,
            #                 test_acc2=test_acc2, test_loss2=test_loss2, )
            # results2 = dict(accuracy_2=accuracy_2, test_loss_2=test_loss_2, precision_2=precision_2, recall_2=recall_2, f1_2=f1_2)
